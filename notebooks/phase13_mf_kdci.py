...
def main():
    ...
    # Step 2: Weighted SVD via Gram matrix
    t2 = time.perf_counter()
    print("  Computing weighted SVD via Gram matrix...", flush=True)
    # Weights = A^{1/2} applied to already-A-weighted columns
    # Actually: hqp_cols = A · H_QP. We need T = A^{1/2} · H_QP.
    # T = A^{-1/2} · (A · H_QP)  ... no, that requires diagonal elements.
    # Simpler: compute T directly by building A^{1/2} · H_QP columns
    t_hqp = build_hqp_sparse(p_dets, ham, sqrt_A_func, N_ACT, skip_P=p_set)

    # Weights are 1.0 since sqrt_A is already in t_hqp
    compressed, sigmas, r = gram_svd(t_hqp, threshold=1e-3)
    del t_hqp  # free memory
    print(f"    {N} columns → {r} compressed vectors "
          f"(σ: {sigmas[0]:.3f} → {sigmas[-1]:.3f}, "
          f"{time.perf_counter()-t2:.0f}s)", flush=True)

    # Step 3: MGS on compressed vectors
    U = sparse_mgs(compressed, [])
    r = len(U)
    print(f"    MGS: {len(compressed)} → {r} orthonormal basis vectors", flush=True)
