#!/usr/bin/env python3
"""
Phase 6b: Krylov Convergence — FIXED propagation + full-CAS Q space

Fix 1: AB acts on compressed basis (U_orth), not raw vectors
Fix 2: Use full CAS(8,8) as Q-space (via cistring) instead of Q-neighborhood,
        to ensure fixed occupation for make_hdiag
...
def build_sparse_h_qq(ham, qa_strs, qb_strs, norb):
    """Build H_QQ as a sparse adjacency list."""
    na, nb = len(qa_strs), len(qb_strs)
    M = na * nb
    off_diag = [[] for _ in range(M)]
    qa_list = list(qa_strs); qb_list = list(qb_strs)
    qa_map = {int(s): i for i, s in enumerate(qa_list)}
    qb_map = {int(s): i for i, s in enumerate(qb_list)}
    nnz = 0
    for idx_a in range(na):
        for idx_b in range(nb):
            i = idx_a * nb + idx_b
            a_str, b_str = int(qa_list[idx_a]), int(qb_list[idx_b])
...
def main():
    print("=" * 80)
    print("Phase 6b: Krylov Convergence — FIXED+full-CAS Q, sparse H_QQ, CAS(8,8)")
    print("=" * 80)
    ...
    for R in BOND_LENGTHS:
        ...
        norb=N_ACT; nelec=(na,nb)

        # === Full CAS(8,8) as Q-space (fixed occupation) ===
        qa = cistring.gen_strings4orblist(list(range(norb)), nelec[0])
        qb = cistring.gen_strings4orblist(list(range(norb)), nelec[1])
        ...
        # === H_D' via PySCF (full CAS strings) ===
        ci_strs=(qa,qb)
        hdiag=selected_ci.make_hdiag(h1_act,eri_packed,ci_strs,norb,nelec)
        print(f"  H_D' computed for {M} dets: {hdiag.min():.6f} ~ {hdiag.max():.6f} Ha")
        ...
        print(f"  H_QP built: {np.count_nonzero(H_QP)} nnz, {t_hqp:.1f}s")
        ...
        # ============================================================
        # Krylov layers — FIXED: propagate compressed basis
        # ============================================================
