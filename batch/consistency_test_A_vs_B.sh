...
echo "============================================"
echo "Scheme A vs B Consistency Test (gs, m_max=0)"
...
print()
dE_a = a.get('dE_m0_mH', 0)
dE_b = b.get('dE_m0_mH', 0)
print(f'  dE(m=0) A = {dE_a:+.6f} mH  vs B = {dE_b:+.6f} mH')
if abs(dE_a - dE_b) < 1e-4:
    print('  PASS: Scheme B matches Scheme A within 1e-4 mH')
else:
    print('  FAIL: Significant discrepancy!')
    all_ok = False
