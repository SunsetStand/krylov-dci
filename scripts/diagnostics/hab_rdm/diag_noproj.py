#!/usr/bin/env python3
"""Compare H2 (sigma) vs R3+R13 (contraction) WITHOUT projection, element-wise."""
import numpy as np
import sys
...
# NO projection: compare raw
mask = compatible_product_mask(schmidt, partition, (na, nb))
print(f"physical mask: {mask.sum()}/{D} True")
...
d = np.abs(rdm - ref)
print(f"2->3 NO-projection: max|rdm-ref| = {d.max():.3e}")
# which elements are large?
...
# check if the diff elements are in the physical (mask) rows/cols
# row idx within 2->3 block = global bo[nd]+i ; col = bo[ns]+j
print("\nPhysical-subspace status of top-diff elements:")
for idx in flat[:6]:
    i, j = np.unravel_index(idx, d.shape)
    row_phys = mask[bo[nd]+i]; col_phys = mask[bo[ns]+j]
    print(f"  [{i},{j}] row_phys={row_phys} col_phys={col_phys} diff={rdm[i,j]-ref[i,j]:+.6f}")
