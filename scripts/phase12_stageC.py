#!/usr/bin/env python3
"""
Phase 12 — Stage C: P×m Convergence with HF Perturbation Selection.

Key differences from Stage B:
  - P-space: HF-based perturbative selection (no FCI CI coefficients)
  - Reference: DMRG-CI (maxM=500, nroots=6)
  - level_shift = 0 (removed — it broke Krylov convergence direction)

Perturbation criterion for determinant |D⟩:
  score = |⟨D|H|HF⟩|² / (E_HF − H_DD)
Select top-P determinants by descending |score|.

Usage:
    python phase12_stageC.py --P 200 --nproc 4 --ckpt-dir ./checkpoints_stageC
"""
...
def parse_args():
    ...
    p.add_argument('--ckpt-dir', type=str, default='./checkpoints_stageC')
...
def _worker_hqp(args):
    ...
    for p_idx in range(p_start, p_end):
        ...
        ao_l = [k for k in range(n_act) if (pa>>k)&1]; av_l = [k for k in range(n_act) if k not in ao_l]
        bo_l = [k for k in range(n_act) if (pb>>k)&1]; bv_l = [k for k in range(n_act) if k not in bo_l]
        conn = set()
        for ii in ao_l:
            for v in av_l: conn.add(((pa^(1<<ii))|(1<<v), pb))
        for ii in bo_l:
            for v in bv_l: conn.add((pa, (pb^(1<<ii))|(1<<v)))
        nao, nav = len(ao_l), len(av_l); nbo, nbv = len(bo_l), len(bv_l)
        if nao >= 2:
...
# ── Global data: DMRG-CI reference + H_QQ (cached once) ─────────────

def get_global_data(mol, mf, ckpt_dir, force, nroots, nproc):
    """Compute or load DMRG-CI reference and H_QQ sparse matrix."""
    cache = 'global_data'
    if not force and _cp_exists(ckpt_dir, 0, cache):
        print("Loading global DMRG-CI + H_QQ from cache...", flush=True)
        d = _cp_load(ckpt_dir, 0, cache)
        return (d['e_dmrg'], d['e_dmrg_bare'], d['qa'], d['qb'],
    ...
    # ── DMRG-CI reference ──
    print(f"DMRG-CI reference (CAS({N_ACT},{N_ELEC}), maxM={DMRG_MAXM}, nroots={nroots})...", flush=True)
    ...
    mc.fcisolver = solver
    mc.kernel()
    e_dmrg = np.asarray(mc.e_tot)

    # Get frozen-core-corrected integrals (consistent with DMRG-CI)
    cas = mcscf.CASCI(mf, N_ACT, N_ELEC)
    cas.frozen = N_CORE; cas.mo_coeff = mf.mo_coeff
    h1eff, ecore = cas.get_h1eff()
    h2eff = cas.get_h2eff()
    ecore = float(ecore)
    e_dmrg_bare = e_dmrg - ecore
    ...
    print(f"  DMRG-CI done ({time.perf_counter()-t0:.0f}s)", flush=True)
    print(f"  DMRG-CI E₀ = {e_dmrg[0]:.8f} Ha", flush=True)
    for i in range(1, min(nroots, len(e_dmrg))):
        print(f"  DMRG-CI E{i} = {e_dmrg[i]:.8f} Ha  ({1000*(e_dmrg[i]-e_dmrg[0]):.1f} mH)", flush=True)

    # ── H_QQ adjacency (parallel) ──
    h2_4d = _unpack_4fold(h2eff, N_ACT)
    hdiag = selected_ci.make_hdiag(h1eff, h2eff, (qa, qb), N_ACT, nelec)
    ...
    _cp_save(ckpt_dir, 0, cache,
             e_dmrg=e_dmrg, e_dmrg_bare=e_dmrg_bare,
             qa=qa, qb=qb, ecore=ecore, na=na, nb=nb,
             h1eff=h1eff, h2eff_packed=h2eff, hdiag=hdiag,
             H_QQ_data=H_QQ.data, H_QQ_indices=H_QQ.indices,
             H_QQ_indptr=H_QQ.indptr, M=M)
    return (e_dmrg, e_dmrg_bare, qa, qb, ecore, na, nb,
            hdiag, H_QQ.data, H_QQ.indices, H_QQ.indptr, M)


# ── P-space selection: HF perturbative criterion ────────────────────

def setup_p_space_perturbative(h1eff, h2_4d, P_target, ckpt_dir, force):
    """Select P determinants via HF-based perturbation theory.

    |D⟩ is scored by:  score = |⟨D|H|HF⟩|² / (E_HF − H_DD)
    Top-P determinants by descending |score| are selected.

    For canonical HF orbitals, single-excitation couplings vanish by
    Brillouin's theorem, so only double excitations contribute.
    """
    cache = 'p_space'
    if not force and _cp_exists(ckpt_dir, P_target, cache):
        d = _cp_load(ckpt_dir, P_target, cache)
        print(f"  Loading P-space from cache ({int(d['N'])} dets)...", flush=True)
        return d['p_alpha'], d['p_beta'], int(d['N'])

    print("  Selecting P-space via HF perturbation theory...", flush=True)
    t0 = time.perf_counter()

    from determinants import hf_determinant, bit_positions
    ham = Hamiltonian(h1=h1eff, h2=h2_4d, E_nuc=0.0, E_HF=0.0)
    na = nb = N_ELEC // 2
    hf_alpha, hf_beta = hf_determinant(na, nb)

    E_HF = ham.diagonal_element(hf_alpha, hf_beta)

    # Generate all double excitations from HF
    alpha_occ = bit_positions(hf_alpha)
    beta_occ = bit_positions(hf_beta)
            ...
            for j in alpha_occ[idx_i + 1:]:
                for idx_a, a in enumerate(alpha_virt):
                    for b in alpha_virt[idx_a + 1:]:
                        new_a = (hf_alpha ^ (1<<i) ^ (1<<j)) | (1<<a) | (1<<b)
                        det = (new_a, hf_beta)
                        hij = ham.matrix_element((hf_alpha, hf_beta), det)
                        hdd = ham.diagonal_element(det[0], det[1])
                        denom = E_HF - hdd
                        if abs(denom) > 1e-12:
                            scores.append((det, -(hij*hij)/denom))  # negative → sort descending
    ...
    for i in alpha_occ:
        for j in beta_occ:
            for a in alpha_virt:
                for b in beta_virt:
                    new_a = (hf_alpha ^ (1<<i)) | (1<<a)
                    new_b = (hf_beta ^ (1<<j)) | (1<<b)
                    det = (new_a, new_b)
                    hij = ham.matrix_element((hf_alpha, hf_beta), det)
                    hdd = ham.diagonal_element(det[0], det[1])
                    denom = E_HF - hdd
    ...
    # Sort by score descending (most negative = largest perturbation contribution)
    scores.sort(key=lambda x: x[1], reverse=True)

    # Select top P (always include HF as the first determinant)
    P_actual = min(P_target - 1, len(scores))
    p_dets = [(hf_alpha, hf_beta)] + [d for d, _ in scores[:P_actual]]
    N = len(p_dets)

    print(f"  Selected P={N} dets from {len(scores)} SD excitations "
    ...
    _cp_save(ckpt_dir, P_target, cache,
             ...
             N=N)
    return (np.array([d[0] for d in p_dets]), np.array([d[1] for d in p_dets]), N)
...
def setup_p_blocks(ckpt_dir, P_target, force, nproc):
    ...
    h1eff = gd['h1eff']; h2eff_p = gd['h2eff_packed']
    h2_4d = _unpack_4fold(h2eff_p, N_ACT)
    ham = Hamiltonian(h1=h1eff, h2=h2_4d, E_nuc=0.0, E_HF=0.0)
...
def run_krylov(P_target, ckpt_dir, m_max, force, nroots):
    ...
    delta_ref = E0_P - e_dmrg_bare[0]
    A_diag = 1.0/(E0_P - hdiag)  # no level shift
    dE0_P = (E0_P - e_dmrg_bare[0])*1000

    def sigma(X):
        return H_QQ @ X

    print(f"\n{'─'*70}\nP={N:4d}  M={M:,}  M/N={M/N:.0f}  "
          f"E₀(P)−E(DMRG)={dE0_P:+.1f} mH\n{'─'*70}")
    ...
    for m in range(m_max+1):
        ...
        H_eff = build_effective_H(H_PP, H_PQ_t, H_QQ_t, E0_P, delta=use_d)  # no level shift
        ev, _ = diagonalize_effective_H(H_eff, n_states=None)
        ev = ev[:min(nroots, len(ev))]
        ef_sub = e_dmrg_bare[:len(ev)]
    ...
    print(f"  {'St':>3s}  {'E(DMRG)':>16s}  {'E(kDCI)':>16s}  {'Δ(mH)':>10s}")
    for st in range(nr):
        print(f"  {st:3d}  {ef_sub[st]+ecore:16.8f}  "
              f"{results[-1]['ev'][st]+ecore:16.8f}  {1000*(results[-1]['ev'][st]-ef_sub[st]):+10.1f}")
    with open(os.path.join(_cp_dir(ckpt_dir, P_target), 'summary.json'),'w') as f:
        json.dump({'P':P_target,'N':N,'M':M,'dE0_P_ref_mH':float(dE0_P),
                   'e_dmrg_total':[float(x+ecore) for x in ef_sub[:nr]],
...
def main():
    ...
    print(f"{'='*70}\nPhase 12 Stage C  P={a.P}  m_max={a.m_max}  "
          f"nroots={a.nroots}  nproc={nproc}  ls=0\n"
          f"  P-space: HF perturbative selection\n"
          f"  Ref: DMRG-CI(maxM={DMRG_MAXM})\n"
    ...
    e_dmrg, e_dmrg_bare, qa, qb, ecore, na, nb, hdiag, hd, hi, hp, M = \
        get_global_data(mol, mf, a.ckpt_dir, a.force, a.nroots, nproc)

    # P-space: HF perturbation selection
    gd = _cp_load(a.ckpt_dir, 0, 'global_data')
    h2_4d = _unpack_4fold(gd['h2eff_packed'], N_ACT)
    setup_p_space_perturbative(gd['h1eff'], h2_4d, a.P, a.ckpt_dir, a.force)
