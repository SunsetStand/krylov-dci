...
└── SVD compression         ← Gram SVD for basis reduction
```
...
9. **SVD compression is ineffective when all columns are independent.** For N=200, M=63,504, the column space dimension equals N (no linear dependence). SVD value emerges when N > column rank or when aggressive truncation is needed.
