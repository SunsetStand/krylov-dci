...
def eval_checkpoint(p_dets, p_full_idx, H_PP_sub, p_target, it_num):
    ...
    global LAST_U1_TRUNC; LAST_U1_TRUNC = kr_results[1]["U"] if len(kr_results) > 1 else None
    global LAST_P_TRUNC, LAST_HPP_TRUNC, LAST_EREFS_TRUNC; LAST_P_TRUNC = list(p_dets); LAST_HPP_TRUNC = H_PP_sub.copy(); LAST_EREFS_TRUNC = E0_vals[:NROOTS].copy()
...
# ======================================================================
# Post-hoc m=1 SVD truncation sweep (add-on to v10_sacis pipeline)
# ======================================================================
global LAST_U1_TRUNC, LAST_P_TRUNC, LAST_HPP_TRUNC, LAST_EREFS_TRUNC
if "LAST_U1_TRUNC" in dir() and LAST_U1_TRUNC is not None:
    print("\\n" + "="*70, flush=True)
    print("Post-hoc SVD Truncation Sweep (m=1)", flush=True)
    print("="*70, flush=True)
    
    U_1 = LAST_U1_TRUNC
    d_full = U_1.shape[1]
    p_dets_s = LAST_P_TRUNC
    H_PP_s = LAST_HPP_TRUNC
    E_refs_s = LAST_EREFS_TRUNC
    Np = len(p_dets_s)
    ...
    for k in range(d_full):
        ...
        if (k+1) % max(1, d_full//5) == 0:
            print(f"    {k+1}/{d_full} ({time.perf_counter()-t_sig:.0f}s)", flush=True)
    ...
    sigma_prop  = sigma_prop / sigma_prop.max()
    sigma_combined = np.concatenate([sigma_build, sigma_prop])
    # Sort
    sort_idx = np.argsort(-sigma_combined[:d_full])
    U_sorted = U_1[:, sort_idx]
    SIG_sorted = SIG[:, sort_idx]
    sigma_sorted = sigma_combined[sort_idx]
    
    p_flat = kdci_sparse.q_idx.p_indices(p_dets_s)
    p_valid = p_flat >= 0; p_f = p_flat[p_valid]
    ...
    for thr in THRESHOLDS:
        ...
        if r == 0:
            print("  {:>8.0e} {:>5}  (skip)".format(thr, 0), flush=True); continue
        U_r = U_sorted[:, :r]; SIG_r = SIG_sorted[:, :r]
        H_KK_r = U_r.T @ SIG_r; H_KK_r = 0.5*(H_KK_r + H_KK_r.T)
        H_PK_r = np.zeros((Np, r))
        H_PK_r[p_valid, :] = SIG_r[p_f, :]
        ev = perstate_eff_eigvals(H_PP_s, H_PK_r, H_KK_r, E_refs_s, NROOTS)
        dE = [(ev[k] - e_fci[k])*1000 for k in range(min(NROOTS, len(ev)))]
        compr = 100.0*(1.0 - r/d_full)
        print("  {:>8.0e} {:>5} {:>7.1f}% {:>+9.3f} {:>+9.3f} {:>+9.3f} {:>+9.3f}".format(thr, r, compr, dE[0], dE[1], dE[2], dE[3]), flush=True)
    
    outdir_tr = os.path.join(PROJECT_ROOT, "checkpoints_phaseA")
    os.makedirs(outdir_tr, exist_ok=True)
    trunc_path = "{}/cas10_trunc_dE_P{}_m1_cis.json".format(outdir_tr, TARGET_P)
    with open(trunc_path, "w") as f:
        json.dump({"config": {"target_p": TARGET_P, "d_full": int(d_full), "e_fci": e_fci},
                   "results": []}, f, indent=2)
    print("\\nSaved: {}".format(trunc_path), flush=True)
